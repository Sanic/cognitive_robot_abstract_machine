...
class CameraPoseGenerationEngine:
    ...
    def generate_camera_poses_from_mean_normals(
        ...
        *,
        ...
        test_fov: tuple[float, float] | None = None,
        fit_method: str = "footprint_2d",
        ...
        class MeanNormalCameraPlacer:
            ...
            @staticmethod
            def _new_camera_instance(test_fov: tuple[float, float] | None) -> Camera:
            ...
            def process_node(
                ...
                pose_cam = self._new_camera_instance(self.test_fov)
                pose_cam.z_near = 0.1
        ...
        placer = MeanNormalCameraPlacer(config, test_fov=test_fov)
    ...
    def generate_poses(self):
        ...
        generated_camera_poses = (
            CameraPoseGenerationEngine.generate_camera_poses_from_mean_normals(
                ...
                test_fov=(float(self.test_fov[0]), float(self.test_fov[1])),
            )
        )
        ...
        cone_cfg = CameraPoseGenerationEngine.ConeSamplingConfig(
            ...
            reject_overlapping_cones=False,  # Set to True for significantly faster compute, but way fewer candidates
        )
        ...
        _extra_cone_poses = CameraPoseGenerationEngine.generate_cone_view_poses(
            ...
            fov_deg_xy=(float(self.test_fov[0]), float(self.test_fov[1])),
            visualize=False,
            marker_height=0.15,
        )
        ...
        if self.debug_visibility_reports:
